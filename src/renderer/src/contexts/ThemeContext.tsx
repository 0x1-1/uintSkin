...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const getStoredConfig = () => {
    ...
    if (stored) {
      try {
        ...
        // Map legacy theme ids to the new brand id
        if (parsed.themeId === 'default') {
          parsed.themeId = 'mythic'
        }
  ...
  const [themeConfig, setThemeConfig] = useState<ThemeConfig>(() => {
    ...
    if (legacyDarkMode === 'true') {
      return { themeId: 'mythic', mode: 'dark' }
    } else if (legacyDarkMode === 'false') {
      return { themeId: 'mythic', mode: 'light' }
    }

    // Default to system preference
    return { themeId: 'mythic', mode: 'system' }
  })
