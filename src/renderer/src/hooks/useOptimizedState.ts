...
export const useStyles = () => {
  return useMemo(
    () => ({
      ...
      dropOverlay: {
        className:
          'fixed inset-0 bg-[#050914] bg-opacity-85 flex items-center justify-center z-50 pointer-events-none backdrop-blur-md'
      },
      mainContainer: {
        className:
          'relative flex flex-col h-screen pt-12 bg-background/90 text-text-primary overflow-hidden transition-colors duration-300'
      },
      toolsModalOverlay: {
        className:
          'fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-md flex items-center justify-center z-50 animate-fade-in'
      },
      toolsModalContent: {
        className:
